...
${join("\n", formatlist("%s ansible_host=%s ansible_user=ubuntu ansible_ssh_private_key_file=${var.ssh-private-key}", azurerm_linux_virtual_machine.worker-nodes.*.name, azurerm_linux_virtual_machine.worker-nodes.*.private_ip_address))}
[hpc-master]
